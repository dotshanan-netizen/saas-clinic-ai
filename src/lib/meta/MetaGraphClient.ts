...
export class MetaGraphClient {
  ...
  static async fetchWabas(token: string, businessId: string) {
    const res = await ConnectionManager.withFetchResilience(
      this.getUrl(`${businessId}/owned_whatsapp_business_accounts`), 
  ...
  static async fetchPhones(token: string, wabaId: string) {
    const res = await ConnectionManager.withFetchResilience(
      this.getUrl(`${wabaId}/phone_numbers`), 
      { headers: { 'Authorization': `Bearer ${token}` } },
      "MetaGraph API"
    );
    return res.json();
  ...
  static async subscribeApp(token: string, wabaId: string) {
    const res = await ConnectionManager.withFetchResilience(
      this.getUrl(`${wabaId}/subscribed_apps`), 
      {
        method: "POST",
        headers: { 'Authorization': `Bearer ${token}` }
      },
  ...
  static async verifyPhone(token: string, phoneId: string) {
    const res = await ConnectionManager.withFetchResilience(
      this.getUrl(`${phoneId}?fields=status,verified_name`), 
      { headers: { 'Authorization': `Bearer ${token}` } },
      "MetaGraph API"
    );
    return res.json();
